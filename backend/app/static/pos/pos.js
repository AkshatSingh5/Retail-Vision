...
function renderFoundCard(product) {
  ...
  const img = product.image_url
    ? `<img src="${product.image_url}?t=${Date.now()}" alt="${product.name}" onerror="this.style.display='none'" />`
    : "";
...
function renderActionCard({ title, message, previewUrl, state, allowAddNew = false, className = "missing" }) {
  ...
  const preview = previewUrl
    ? `<img src="${previewUrl}?t=${Date.now()}" alt="Captured product" onerror="this.style.display='none'" />`
    : "";
  card.innerHTML = `
    <strong>${title}</strong>
    ${preview}
...
function openRegisterFromScan(payload) {
  ...
  if (payload.preview_url) {
    els.productPreview.src = `${payload.preview_url}?t=${Date.now()}`;
  } else {
...
async function submitRegisterProduct(forceCreate) {
  ...
  const response = await fetch("/products/register", { method: "POST", body: data });
...
function showSimilarProduct(detail) {
  ...
  if (product.image_url) {
    els.similarPreview.src = `${product.image_url}?t=${Date.now()}`;
...
els.btnBill.addEventListener("click", async () => {
  ...
  try {
    ...
    els.modalPdf.href = result.pdf_url;
